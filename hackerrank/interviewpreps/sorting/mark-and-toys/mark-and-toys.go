...
func maximumToys(prices []int32, k int32) int32 {
	pricesInt := make([]int, len(prices), len(prices))
	// convert int32 to int and sort it
	for i := 0; i < len(prices); i++ {
		pricesInt[i] = int(prices[i])
	}
	sort.Ints(pricesInt)

	sum := 0
	result := 0
	for result = 0; result < len(prices); result++ {
		sum += pricesInt[result]
		if sum > int(k) {
			break
		}
	}
